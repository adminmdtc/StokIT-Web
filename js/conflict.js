...
const ConflictResolver = {
  ...
  detectConflicts(remoteData) {
    ...
    /* ถ้า remoteData เท่ากับ base → ไม่มีการเปลี่ยนแปลง */
    const remoteJson = JSON.stringify(remoteData);
    const baseJson = JSON.stringify(base);
    if (remoteJson === baseJson) {
      return { hasConflict: false, canAutoMerge: true, conflicts: [], autoMergeable: [] };
    }

    /* ถ้า local ไม่มีการเปลี่ยนแปลง → ใช้ remote */
    const localJson = JSON.stringify(Store.db);
    if (localJson === baseJson) {
      return { hasConflict: false, canAutoMerge: true, conflicts: [], autoMergeable: [], useRemote: true };
    }
