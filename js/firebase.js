...
const FirebaseDB = {
  ...
  /* ดึงข้อมูลที่มีเฉพาะในเครื่องนี้ (ยังไม่ได้ sync ขึ้น Firebase) */
  ...
  /* ซิงค์ข้อมูลจาก Firebase ลง local — ง่ายๆ: ใช้ remote + เก็บ local-only */
  async syncFromFirebase() {
    ...
    try {
      ...
      if (remoteData && typeof remoteData.items === 'object') {
        ...
        // ใช้ข้อมูล remote เป็นหลัก
        Store.db = remoteData;
        
        // เพิ่มข้อมูลท้องถิ่นที่ยังไม่มีใน remote กลับเข้าไป
  ...
  /* ฟังการเปลี่ยนแปลงแบบ real-time — ง่ายๆ: รับ data แล้ว merge */
  onChanges(callback) {
    ...
    ref.on('value', async (snapshot) => {
      ...
      // ถ้าเพิ่ง syncToFirebase ให้ข้าม (เป็น echo ของตัวเอง)
      if (this._justSynced) {
        this._justSynced = false;
        const timeDiff = Date.now() - (this._justSyncedTime || 0);
        if (timeDiff < 5000) {
          console.log('Firebase echo after syncTo — skip');
          return;
        }
      }
      ...
      if (currentJson === newJson) {
        console.log('Firebase data unchanged — skip');
        return;
      }
      
      console.log('Firebase real-time update received');
      
      // ดึงข้อมูลท้องถิ่นที่ยังไม่ได้ sync
      const localOnly = this._getLocalOnlyData(data);
      
      // ใช้ข้อมูล remote เป็นหลัก
      Store.db = data;
      
      // เพิ่มข้อมูลท้องถิ่นที่ยังไม่มีใน remote
      if (!Store.db.items) Store.db.items = [];
      if (!Store.db.transactions) Store.db.transactions = [];
      if (!Store.db.users) Store.db.users = [];
...
async function autoSyncToFirebase() {
  if (isFirebaseEnabled()) {
    try {
      return await FirebaseDB.syncToFirebase();
    } catch (e) {
