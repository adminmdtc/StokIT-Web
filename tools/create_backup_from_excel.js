...
// Read new Excel report (Sept 1, 2026)
const wb = XLSX.readFile('D:/รายการStok/รายงานคงเหลือวัสดุ_2026-09-01.xlsx');
const ws = wb.Sheets[wb.SheetNames[0]];
const excelData = XLSX.utils.sheet_to_json(ws);

console.log('Excel rows:', excelData.length);

// Current seed items from store.js (i1-i62)
const seedItems = [
  ...
  /* หมวดหมู่ใหม่ (i50-i62) */
  { id: 'i50', code: 'RAM-DDR4-8G', name: 'แรม DDR4 8GB 2666MHz', category: 'อะไหล่วัสดุคอมพิวเตอร์', unit: 'ชิ้น', mission: 'm4', group: 'g12' },
  { id: 'i51', code: 'HDD-1TB', name: 'ฮาร์ดดิส Western Digital 1TB', category: 'อะไหล่วัสดุคอมพิวเตอร์', unit: 'ชิ้น', mission: 'm4', group: 'g12' },
  { id: 'i52', code: 'SSD-256G', name: 'SSD 256GB SATA 2.5"', category: 'อะไหล่วัสดุคอมพิวเตอร์', unit: 'ชิ้น', mission: 'm4', group: 'g12' },
  { id: 'i53', code: 'PSU-500W', name: 'พาวเวอร์ซัพพลาย 500W', category: 'อะไหล่วัสดุคอมพิวเตอร์', unit: 'ชิ้น', mission: 'm4', group: 'g12' },
  { id: 'i54', code: 'SCREW-KIT', name: 'ชุดไขควง 6 แกน', category: 'เครื่องมือช่าง', unit: 'ชุด', mission: 'm4', group: 'g12' },
  { id: 'i55', code: 'PLIER-SET', name: 'ชุดคีม 3 ขนาด', category: 'เครื่องมือช่าง', unit: 'ชุด', mission: 'm4', group: 'g12' },
  { id: 'i56', code: 'TAPE-INSUL', name: 'เทปพันสายไฟ 19mm', category: 'เครื่องมือช่าง', unit: 'ม้วน', mission: 'm4', group: 'g12' },
  { id: 'i57', code: 'DRUM-PANTUM', name: 'ดรั้ม Panther DR-2200', category: 'อะไหล่วัสดุปริ้นเตอร์', unit: 'ชิ้น', mission: 'm1', group: 'g1' },
  { id: 'i58', code: 'FUSER-HP', name: 'ชุดความร้อน Fuser Unit HP M402', category: 'อะไหล่วัสดุปริ้นเตอร์', unit: 'ชิ้น', mission: 'm1', group: 'g1' },
  { id: 'i59', code: 'ROLLER-EPSON', name: 'ยางม้วนกระดาษ Epson L3110', category: 'อะไหล่วัสดุปริ้นเตอร์', unit: 'ชิ้น', mission: 'm1', group: 'g1' },
  { id: 'i60', code: 'CLEAN-CART', name: 'ชุดทำความสะอาดหัวพิมพ์', category: 'วัสดุอุปกรณ์บำรุงต่างๆ', unit: 'ชุด', mission: 'm1', group: 'g1' },
  { id: 'i61', code: 'SPRAY-AIR', name: 'สเปรย์เป่าฝุ่น 400ml', category: 'วัสดุอุปกรณ์บำรุงต่างๆ', unit: 'กระป๋อง', mission: 'm4', group: 'g12' },
  { id: 'i62', code: 'THERMAL-PASTE', name: 'ซีลิโคนระบายความร้อน Arctic MX-4', category: 'วัสดุอุปกรณ์บำรุงต่างๆ', unit: 'หลอด', mission: 'm4', group: 'g12' },
];

// Build code -> seed mapping
const codeToSeed = {};
seedItems.forEach(s => { codeToSeed[s.code] = s; });

// Track matched/unmatched
let matched = 0, unmatched = 0, newItems = 0;
const transactions = [];
let rcvNo = 1;
const Y = 2026;
...
excelData.forEach(row => {
  ...
  const name = String(row['รายการวัสดุ'] || '').trim();
  ...
  if (seed) {
    ...
    if (qty > 0) {
      transactions.push({
        ...
        note: 'ข้อมูลจากรายงานคงเหลือ 01/09/2569',
        mission: seed.mission, group: seed.group,
        items: [{ itemId: seed.id, name: seed.name, qty, serials: [] }],
      });
    }
  } else {
    // New item not in seed - will be added as new item
    newItems++;
    const newId = 'new_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2,6);
    if (qty > 0) {
      transactions.push({
        id: 'tx_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2,6),
        type: 'receive',
        ...
        by: 'admin', byName: 'Admin001',
        note: 'ข้อมูลจากรายงานคงเหลือ 01/09/2569',
        mission: 'm4', group: 'g12',
        items: [{ itemId: newId, name, qty, serials: [] }],
      });
    }
    // Add new item to seedItems for items array
    seedItems.push({
      id: newId, code, name, category, unit,
      mission: 'm4', group: 'g12',
    });
  }
});

// Build final items array (all seed + new)
const allItems = seedItems.map(s => ({
...
const backup = {
  ...
  users,
  reorderItems: [],
  seq: { item: allItems.length, receive: rcvNo - 1, issue: 0 },
};

fs.writeFileSync('IT-Stock-Backup-2026-09-01.json', JSON.stringify(backup, null, 2));

// Verify stock
const stockMap = {};
allItems.forEach(i => stockMap[i.id] = { name: i.name, qty: 0 });
